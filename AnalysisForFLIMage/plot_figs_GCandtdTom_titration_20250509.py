...
for each_firstfilepath in one_of_file_list:
    ...
    for nth, file_path in enumerate(filelist):
        ...
        if imagearray.shape == imageshape:
            Zstack_nth_list.append(nth)
            
        elif (imagearray.shape[0] > 29):
                print(file_path,'<- uncaging')
                uncaging_nth_list.append(nth)
        elif (imagearray.shape[0] == 4):
    ...
    Zstack_nth_array = np.array(Zstack_nth_list)
    for number, nth in enumerate(uncaging_nth_list[:-1]):
        zstack_before_uncaging_nth =  Zstack_nth_array[Zstack_nth_array < nth].max()
        zstack_before_uncaging_n_plus1_th =  Zstack_nth_array[Zstack_nth_array < uncaging_nth_list[number + 1]].max()
    ...
    if len(uncaging_nth_list)>0:
        ...
        zstack_before_uncaging_nth =  Zstack_nth_array[Zstack_nth_array < uncaging_nth_list[-1]].max()

        pre_post_set_list.append({
            "pre":filelist[zstack_before_uncaging_nth],
